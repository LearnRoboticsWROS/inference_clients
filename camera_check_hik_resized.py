...
WIN_NAME = "Hikrobot (q: esci, f: fit/zoom, +/-: zoom)"
...
def draw_info(img, fps=None, zoom_txt=""):
    if fps is not None:
...
def work_thread(cam):
    ...
    # lascia che l’utente ridimensioni la finestra; in modalità FIT l’immagine segue la finestra
    ...
    while not g_bExit:
        ...
        try:
            ...
            if n % 10 == 0:
                ...
                t0, n = time.time(), 0  # azzera per media mobile

            view = img
            if USE_FIT:
                # calcola fit alla finestra corrente
...
if __name__ == "__main__":
    ...
    try:
        ...
        th.start()
        th.join()  # aspetta finché non premi 'q' nella finestra
    finally:
        g_bExit = True
