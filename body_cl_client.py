...
Body_CL inspection client (HIK Mono8 + Roboflow) aligned to lip_client.py / body_client.py:

- daemon trigger mode (--daemon, --trigger-file, --result-file)
- optional AWS S3 upload of NOK images (raw + annotated) with tags
- headless mode (--headless) disables only OpenCV GUI (imshow / waitKey)
- IMPORTANT: annotated image saved to S3 ALWAYS contains:
  - bounding boxes
...
DEFAULT_MODEL_ID = "big400-body-cl-insp-before-cleaning/1"
DEFAULT_TH       = 0.30
...
def any_defect(preds, th: float, ok_classes: Optional[List[str]], debug: bool = False):
    ...
    for p in preds["predictions"]:
        ...
        if debug:
            print(f"[debug] class={cls} conf={conf:.3f} th={th:.3f} in_wl={cls in wl}",
...
def parse_args():
    ...
    ap.add_argument("--cam-index", type=int, default=0, help="Indice HIK Body-CL")
...
def main():
    ...
    # === IMPORTANT: always print something at startup (like lip/body) ===
    print(f"[{PREFIX}] START pid={os.getpid()} file={os.path.realpath(__file__)}",
          file=sys.stderr, flush=True)
    print(f"[{PREFIX}] headless={args.headless} save_on_defect={args.save_on_defect} force_upload={args.force_upload} "
          ...
          f"trigger={args.trigger_file} result={args.result_file}",
          file=sys.stderr, flush=True)
    ...
    def hud_lines(mode: str, run_s: float, fps_real: float, expo: int, gain: float,
        ...
        return [
            ...
            f"model={model}",
            f"TH={th:.2f}  infer_ms={infer_ms:.1f}" if infer_ms is not None else f"TH={th:.2f}  infer_ms=--",
        ]
    ...
    def run_window() -> bool:
        ...
        while (time.time() - t0) < args.duration:
            ...
            # IMPORTANT: HUD ALWAYS drawn into the image that may be uploaded
    ...
    try:
        ...
        if not args.daemon:
            res = run_window()
            print("BODY_CL_NOK" if res else "BODY_CL_OK", flush=True)
            sys.exit(0)
        ...
        while True:
            if os.path.exists(args.trigger_file):
                ...
                res = run_window()
                token = "BODY_CL_NOK" if res else "BODY_CL_OK"

                # IMPORTANT: print token to terminal (like lip/body)
